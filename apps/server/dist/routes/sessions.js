...
import { createSessionRequestSchema, updateSessionRequestSchema, sessionResponseSchema } from "@sudoku/contracts";
export async function sessionRoutes(app) {
    ...
    app.get("/api/v1/sessions/:id", async (request, reply) => {
        const session = db
            .select()
            .from(gameSessions)
            .where(and(eq(gameSessions.sessionId, request.params.id), eq(gameSessions.guestId, request.guestId)))
            .get();
        if (!session) {
    ...
    app.put("/api/v1/sessions/:id", async (request, reply) => {
