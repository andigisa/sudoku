...
function mapSession(s: typeof gameSessions.$inferSelect) {
  return {
...
export async function syncRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/sync/bootstrap", async (request, reply) => {
    ...
    const settings: Record<string, string> = sync?.settingsJson ? JSON.parse(sync.settingsJson) : {};
  ...
  app.post("/api/v1/sync/push", async (request, reply) => {
    ...
    const accepted: { index: number; status: "accepted" }[] = [];
    const rejected: { index: number; status: "rejected"; reason: string }[] = [];
    ...
    for (let i = 0; i < changes.length; i++) {
      ...
      if (change.type === "setting_change") {
        ...
        const currentSettings: Record<string, string> = sync?.settingsJson
          ? JSON.parse(sync.settingsJson)
          : {};
        const payload = change.payload as Record<string, string>;
        ...
        const payload = change.payload as { session_id?: string; state_json?: string; elapsed_ms?: number; mistakes?: number; completed_at?: string | null };
        ...
        const session = db
          ...
          .where(
            and(
              eq(gameSessions.sessionId, payload.session_id),
              eq(gameSessions.guestId, request.guestId)
            )
          )
          .get();
