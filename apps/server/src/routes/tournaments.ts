...
function toResponse(t: typeof tournaments.$inferSelect) {
...
function autoActivate(app: FastifyInstance, t: typeof tournaments.$inferSelect) {
...
export async function tournamentRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { id: string } }>("/api/v1/tournaments/:id", async (request, reply) => {
  ...
  app.post<{ Params: { id: string } }>("/api/v1/tournaments/:id/entries", async (request, reply) => {
    ...
    const duplicate = db
      ...
      .where(
        and(
          ...
          eq(tournamentEntries.guestId, guestId),
          eq(tournamentEntries.sessionId, session_id),
          eq(tournamentEntries.elapsedMs, elapsed_ms),
          eq(tournamentEntries.mistakes, mistakes),
          eq(tournamentEntries.hintsUsed, hints_used)
        )
      )
      .get();
      ...
      const snapshot = db
        ...
        .where(
          and(
            ...
            eq(leaderboardSnapshots.guestId, guestId)
          )
        )
        .get();
    ...
    const score = calculateScore(
      session.difficulty as DifficultyDto,
      mistakes,
      hints_used,
      env.TOURNAMENT_RULESET_VERSION
    );
      ...
      for (const r of ranked) {
        db.update(leaderboardSnapshots)
          ...
          .where(
            and(
              eq(leaderboardSnapshots.tournamentId, tournamentId),
              eq(leaderboardSnapshots.guestId, r.guestId)
            )
          )
          .run();
      }
