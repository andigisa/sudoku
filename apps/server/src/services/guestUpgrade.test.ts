...
import { guests, gameSessions, tournamentEntries } from "../db/schema.js";
import { eq } from "drizzle-orm";
import { upgradeGuestToUser } from "./guestUpgrade.js";

function seedGuest(guestId: string) {
  db.insert(guests).values({ guestId, createdAt: new Date().toISOString() }).run();
}

function seedSession(guestId: string, userId: string | null = null) {
  const id = randomUUID();
