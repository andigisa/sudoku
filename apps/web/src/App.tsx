...
export default function App() {
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (event.key >= "1" && event.key <= "9") {
        ...
        handleDigitInput(history.present.selectedCell!, Number(event.key));
  ...
  async function startNewGame(selectedDifficulty: DifficultyDto) {
    ...
    try {
      ...
      solutionRef.current = parseGrid(puzzle.solution);
  ...
  function handleDigitInput(cellIndex: number, digit: number) {
    updateGame((state) => applyInput(state, cellIndex, digit, solutionRef.current ?? undefined));
  }
  ...
  function handleHint() {
    ...
    const correctValue = solutionRef.current[cellIndex];
          ...
          {Array.from({ length: 9 }, (_, index) => {
            ...
            <button
              ...
              onClick={() =>
                ...
                handleDigitInput(history.present.selectedCell!, digit)
              }
              type="button"
            >
              {digit}
